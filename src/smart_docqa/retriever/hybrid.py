...
class HybridRetriever(BaseRetriever):
    ...
    def  _get_relevant_documents(self, query, *, run_manager)->list[Document]:
         dense_docs=self.vector_database.similarity_search(query=query,k=self.k*2,callbacks=run_manager.get_child())
         tokenized_query=query.lower().split()
         bm25_scores=self.bm25.get_scores(tokenized_query)
         bm25_indices=sorted(range(len(bm25_scores)),key=lambda i:bm25_scores[i],reverse=True)[:self.k*2]
         bm25_docs=[self.corpus[i] for i in bm25_indices]
         return self._reciprocal_rank_fusion(dense_docs,bm25_docs)

    def _reciprocal_rank_fusion(self,dense:list[Document],sparse:list[Document],k:int=60)->list[Document]:
         ...
         fusion=sorted(scores,key=lambda i :scores[i],reverse=True)[:self.k]     
         return [doc_map[key] for key in scores]
